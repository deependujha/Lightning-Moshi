...
func makeApiCall(url string, jsonData []byte) (int, error) {
	...
	/// Read the streamed text response in chunks
	buffer := make([]byte, 1024) // 1 KB buffer size for reading text chunks
	for {
		// Read data into buffer
		n, err := resp.Body.Read(buffer)
		if n > 0 {
			// Convert the bytes to a string and process it (for demonstration, print the text)
			textChunk := string(buffer[:n])
