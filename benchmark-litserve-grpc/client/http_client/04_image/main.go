...
func getPayload() (bytes.Buffer, *multipart.Writer) {
	...
	imageFilePath := "carbon.png" // Replace with actual file path
	...
	imageWriter, err := multipartWriter.CreateFormFile("image", "image.png")
	if err != nil {
		log.Fatal("Error writing image file to form:", err)
	}
	_, err = io.Copy(imageWriter, imageFile)
	if err != nil {
		log.Fatal("Error copying image file:", err)
	}
