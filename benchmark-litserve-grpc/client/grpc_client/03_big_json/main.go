...
func main() {
	conn, err := grpc.NewClient("44.203.226.39:8300", grpc.WithTransportCredentials(insecure.NewCredentials()))
	if err != nil {
		panic(err)
	}
	defer conn.Close()
...
func getPayload() *litserve_grpc.Request {
	...
	for i := 1; i <= 100; i++ {
		...
		if field.IsValid() && field.CanSet() && field.Kind() == reflect.String {
			field.SetString("deep" + strconv.Itoa(i))
		}
	}

	// For demonstration, print out the struct
	return req
}
