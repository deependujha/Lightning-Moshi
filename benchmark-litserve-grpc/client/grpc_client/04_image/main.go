...
import (
	...
	"encoding/csv"
	"fmt"
	litserve_grpc "grpc_image/proto"
	"log"
	"os"
	"sync"
	"time"
...
func main() {
	conn, err := grpc.NewClient("44.203.226.39:8400", grpc.WithTransportCredentials(insecure.NewCredentials()))
	if err != nil {
		panic(err)
	}
	defer conn.Close()
...
func getPayload() *litserve_grpc.Request {
	// Read the image file
	imageData, err := os.ReadFile("carbon.png")
	if err != nil {
